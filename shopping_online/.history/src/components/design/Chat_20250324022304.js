...
const Chat = () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        const formattedMessages = data.map((msg, index) => {
          // Xác định loại tin nhắn và áp dụng định dạng phù hợp
          const isUserMessage =
            msg.sender === "user" || (msg.userId && msg.userId === user._id);

          return {
            content: msg.text,
            sender: msg.username || msg.sender,
            id: isUserMessage ? user._id : msg.userId, // Gán id người dùng nếu là tin nhắn của user
            timestamp: msg.timestamp,
            _key: msg._id || `api-msg-${index}-${Date.now()}`,
            isUserMessage: isUserMessage, // Thêm flag để xác định tin nhắn người dùng
          };
        });
  ...
  const renderMessage = ({ item }) => {
    ...
    // hoặc là tin nhắn có sender là "user" hoặc có cờ isUserMessage
    const isMyMessage =
      item.id === user._id ||
      item.isLocalMessage ||
      item.isUserMessage ||
    ...
    // Kiểm tra nếu tin nhắn là từ admin
    const isAdminMessage =
      item.sender === "admin" ||
      (typeof item.sender === "string" &&
        item.sender.toLowerCase().includes("admin"));

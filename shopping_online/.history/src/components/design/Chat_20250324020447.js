...
// API Base URL
const API_URL = "http://localhost:9999/api"; // Điều chỉnh URL API của bạn
// Địa chỉ socket
...
const Chat = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error("Không thể tải tin nhắn từ máy chủ");
      }
  ...
  const sendMessage = async () => {
    if (message.trim()) {
      try {
        ...
        const newMessage = {
          text: message,
          sender: "user", // Hoặc lấy từ thông tin người dùng đã đăng nhập
          username: "current_user", // Thay bằng tên người dùng thực tế
          timestamp: new Date().toISOString(),
        };
  ...
  const clearChatHistory = async () => {
    try {
      ...
      setMessages([]);
      Alert.alert("Thông báo", "Đã xóa lịch sử trò chuyện");
    } catch (error) {
          ...
          <View style={styles.headerButtons}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                Alert.alert(
                  "Xác nhận",
                  "Bạn có chắc muốn xóa lịch sử trò chuyện cục bộ?",
                  [
